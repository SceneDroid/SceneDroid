        ...
        return new FlowFunction<Value>() {
          ...
          public Set<Value> computeTargets(Value source) {
            ...
            if (source.equivTo(zeroValue) && right instanceof StaticFieldRef) {
              ...
              if (PropagationModel.v().getArgumentsForStaticField(rightFieldRef.getField().getSignature()) != null) {
                if (logger.isDebugEnabled()) {
            ...
            if (!source.equivTo(left) && !source.equals(zeroValue)
                && !(source.toString().equals(left.toString()) && PropagationModel.v().isModeledType(type))) {
              res.add(source);
            }
